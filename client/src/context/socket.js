...
// Node/SSR: guard window so this module is safe to import in test environments.
const hasWindow = typeof window !== 'undefined';

export const BACKEND_URL = hasWindow
  ? (import.meta.env?.DEV
      ? `${window.location.protocol}//${window.location.hostname}:3001`
      : window.location.origin)
  : '';

export const socket = hasWindow ? io(BACKEND_URL, { autoConnect: false }) : null;
